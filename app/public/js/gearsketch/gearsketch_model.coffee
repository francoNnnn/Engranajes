...
class Gear
  constructor: (@location, @rotation, @numberOfTeeth, @id,
                @momentum = 0, @group = 0, @level = 0, @connections = {}, @rpm = 0, @initialRpm=0, @arrowHead = null) ->
    @id ?= Util.createUUID()
    ...
    @momentum = @initialRpm*2*Math.PI/60 if @initialRpm
  ...
  clone: ->
    new Gear(@location.clone(), @rotation, @numberOfTeeth, @id,
      @momentum, @group, @level, Util.clone(@connections), @rpm, @initialRpm)

  @fromObject: (obj) ->
    new Gear(Point.fromObject(obj.location), obj.rotation, obj.numberOfTeeth, obj.id,
      obj.momentum, obj.group, obj.level, obj.connections, obj.rpm, obj.rpm)
...
class Board
  ...
  constructor: (@gears = {}, @chains = {}, @inputGear = null, @outputGear = null) ->
  ...
  rotateTurningObjectsFrom: (turningObject,
                             angle, rotatedTurningObjectIds) ->
  ...
  calculateRPM: () ->
    for own id, gear of @gears
      gear.rpm=0
      #console.log("all to zero ")
    for own id, gear of @gears
      if gear.momentum
        #console.log("recalculate momentum "+ gear.momentum)
        @calculateRPMrec(gear, gear.momentum, {})

  calculateRPMrec: (turningObject, momentum, rotatedTurningObjectIds) ->
    unless turningObject.id of rotatedTurningObjectIds
      turningObject.rpm += @momentumToRPM(momentum)######
      #console.log("rpm="+ turningObject.rpm)
      rotatedTurningObjectIds[turningObject.id] = true
    for own neighborId, connectionType of turningObject.connections
      neighbor = @getTurningObjects()[neighborId]
      unless neighborId of rotatedTurningObjectIds
        ratio = @calculateRatio(turningObject, neighbor, connectionType)
        @calculateRPMrec(neighbor, momentum * ratio, rotatedTurningObjectIds)

  momentumToRPM:(momentum)->
    momentum*60/(2*Math.PI)

# check continuity
  getConnectedGearsLength:() ->
    gears = @getGearList()
    if gears.length < 2
      gears.length
    else
      connectedGears={}
      @getConectedObjects(gears[0],connectedGears)
      Object.keys(connectedGears).length


  getConectedObjects:(gear, connectedGears)->
    unless gear.id of connectedGears
      connectedGears[gear.id] = true
    for own neighborId, connectionType of gear.connections
      neighbor = @getTurningObjects()[neighborId]
      unless neighborId of connectedGears
        @getConectedObjects(neighbor,connectedGears)

# label gears

  labelGears: ()->
    path = []
    rootElement = @getInputGear()
    destinationElement = @getOutputGear()
    return path unless (rootElement? and destinationElement?)
    ...
    while queue.length > 0 and not found
      current = queue.shift()
      for own neighborId, connectionType of current.element.connections
        neighbor = graph[neighborId]
        if neighbor.distance is Infinity
          neighbor.distance = current.distance + 1
          neighbor.parent = current.element
          queue.push(neighbor)
          if neighbor.element.id is destinationElement.id
            found = true

    if found
      @getPathFromGraph(graph, destinationElement, path)

    path


  createGraph: (graph = {})->
    for own id, obj of @getTurningObjects()
      graph[obj.id] = new GraphNode(obj)
    graph

  getPathFromGraph:(graph, node, array)->
    @getPathFromGraph(graph, graph[node.id].parent, array) unless graph[node.id].parent is null
    array.push(graph[node.id].element)
  ...
  removeGear: (gear) ->
    delete @inputGear if gear.id is @inputGear
    delete @outputGear if gear.id is @outputGear
    @removeAllConnections(gear)
...
class Game
  ...
  constructor: (@board, @title = "", @rules = {}, @inputs = {}, @modals = {}, @level = 0) ->
    @board ?= new Board()
    #these are just some demo rules
#    @rules.push(new Rule("check_property_equals_sign_bigger_magnitude", {value : -33, property: "rpm", target: "output_gear"}))
#    @rules.push(new Rule("number_of_gears", {value: 5}))
#    @rules.push(new Rule("check_property", {value : 33, property: "numberOfTeeth", target: "input_gear"}))
#    @rules.push(new Rule("check_continuity"))

  validate: ->
    @checksPassed = true
    @labeledGears = @board.labelGears()
    @validateRule rule for id, rule of @rules
    @validateInput input for id, input of @inputs
    console.log if @checksPassed then  "Todas las Validaciones pasaron" else "Oh snap! hubo algun error!"
    console.log "end of validation"
    @checksPassed

  validateRule: (rule) ->
    @checksPassed = @checksPassed and rule.check(@board, @labeledGears)

  validateInput: (input) ->
    @checksPassed = @checksPassed and input.check()

  @fromObject: (obj) ->
    game = new Game()
    game.title = obj.title if(obj.title?)
    game.modals = obj.modals
    game.level = obj.level
    game.board = Board.fromObject(obj.board) if(obj.board)
    for id, rule of obj.rules
      game.rules[id] = Rule.fromObject(rule)
    for id, input of obj.inputs
...
class Rule
  #common properties @value, @property, @target

  RuleType = # type of available rules
  ...
  TargetType =  # used for Check_propperty type rules
    FIRST: "none"
    INPUT_GEAR: "input_gear"
    OUTPUT_GEAR: "output_gear"

  constructor: (@id, @ruleType, @properties)->
    @id ?= Util.createUUID()

  check: (board, labeledGears) ->
    switch @ruleType
      when RuleType.NUMBER_OF_GEARS then return @properties?.value is board.getGearList().length
      when RuleType.CHECK_CONTINUITY then return @continuityCheck board
      when RuleType.CHECK_CHAIN_EXISTENCE then return @chainCheck board
      else return @propertyCheck(board,labeledGears)

  propertyCheck:(board, labeledGears)->
    target = switch @properties?.target
      when TargetType.FIRST then board.getGearList()?[0]
      when TargetType.INPUT_GEAR then board.getInputGear()
      when TargetType.OUTPUT_GEAR then board.getOutputGear()
      else labeledGears[@properties?.target]

    actualValue= if @properties?.useAbsoluteMagnitud?
        Math.abs(target?[@properties?.property])
      else
        target?[@properties?.property]

    switch @ruleType
      when RuleType.CHECK_PROPERTY_EQUALS then Math.round(actualValue*100)/100 is @properties?.value
      when RuleType.CHECK_PROPERTY_NOT_EQUALS then actualValue? and Math.round(actualValue*100)/100 isnt @properties?.value
      when RuleType.CHECK_PROPERTY_LESS_THAN then actualValue < @properties?.value
      when RuleType.CHECK_PROPERTY_GREATER_THAN then actualValue > @properties?.value
      else false
  ...
  chainCheck: (board) ->
    @properties?.value is (Object.keys(board.getChains()).length != 0)

  continuityCheck: (board) ->
    board.getConnectedGearsLength() is board.getGearList().length

  hasNeighbors: (gear) ->
    return Object.keys(gear.connections).length != 0

  @fromObject: (obj) ->
    new Rule(obj.id,obj.ruleType,obj.properties)

window.gearsketch.model.Rule = Rule

class ValidationInput

  InputType = # type of available inputs
    TEXT: "text"
    RADIO: "radio"
    NUMBER: "number"

  # properties = @objective, @beforeInputText, @afterInputText,
  constructor:(@id, @inputType, @correctValue = "", @actualValue = "", @properties = {}) ->
    @id ?= Util.createUUID()

  check: ->
    switch @inputType
      when InputType.TEXT then return @actualValue is @correctValue
      when InputType.NUMBER then return  Number(@actualValue) is Number(@correctValue)
      when InputType.RADIO then return  @actualValue is @correctValue

  @fromObject: (obj) ->
    new ValidationInput(obj.id, obj.inputType, obj.correctValue, obj.actualValue, obj.properties)

window.gearsketch.model.ValidationInput = ValidationInput

class ValidationResult
  constructor:(@level=0, @key="keyasdas", @time=33, @completed=false) ->

window.gearsketch.model.ValidationResult = ValidationResult

class GraphNode
  constructor:(@element, @distance = Infinity, @parent = null)->
